...
bot.on("ready", async () => {
  ...
  bot.user.setActivity(`Music Bot | ?help`, { type: "PLAYING" });
});
...
bot.on("message", async (msg) => { // eslint-disable-line
    ...
    if (command === "play" || command === "p") {
        ...
        if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
            ...
            return msg.channel.send(`:note:  **|**  Playlist: **\`${playlist.title}\`** has been added to the queue!`);
        } else {
            try {
...
Please provide a value to select one of the search results ranging from 1-10.
        ...
        if (!serverQueue) return msg.channel.send("There is nothing playing that I could **\`leave\`** for you.");
        serverQueue.songs = [];
